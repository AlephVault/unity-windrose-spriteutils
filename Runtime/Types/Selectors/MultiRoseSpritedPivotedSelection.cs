...
namespace AlephVault.Unity.WindRose.SpriteUtils
{
    namespace Types
    {
        namespace Selectors
        {
            ...
            /// </summary>
            public class MultiRoseSpritedPivotedSelection : MappedSpriteGridSelection<
                Tuple<MultiSettings<RoseTuple<Vector2Int>>, Vector2>, MultiSettings<SpriteRose>
            >
            {
                public MultiRoseSpritedPivotedSelection(
                    SpriteGrid sourceGrid, Tuple<MultiSettings<RoseTuple<Vector2Int>>, Vector2> selection
                ...
                protected override MultiSettings<SpriteRose> ValidateAndMap(
                    SpriteGrid sourceGrid, Tuple<MultiSettings<RoseTuple<Vector2Int>>, Vector2> selection
                )
                {
                    MultiSettings<SpriteRose> mapping = new MultiSettings<SpriteRose>();
                    foreach (KeyValuePair<State, RoseTuple<Vector2Int>> pair in selection.Item1)
                    {
                ...
                private SpriteRose ValidateAndMapSpriteRose(
                    SpriteGrid sourceGrid, Vector2Int up, Vector2Int down, Vector2Int left,
                    Vector2Int right, Vector2 pivot
                )
                {
                ...
                ~MultiRoseSpritedPivotedSelection()
                {
                    if (result != null)
                    {
