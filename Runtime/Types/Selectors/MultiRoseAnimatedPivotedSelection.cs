...
namespace GameMeanMachine.Unity.WindRose.SpriteUtils
{
    namespace Types
    {
        namespace Selectors
        {
            ...
            /// </summary>
            public class MultiRoseAnimatedPivotedSelection : MappedSpriteGridSelection<
                Tuple<MultiSettings<RoseTuple<ReadOnlyCollection<Vector2Int>>>, Vector2>,
            ...
            >
            {
                ...
                public MultiRoseAnimatedPivotedSelection(
                    SpriteGrid sourceGrid, Tuple<MultiSettings<RoseTuple<ReadOnlyCollection<Vector2Int>>>, Vector2> selection,
                ...
                protected override MultiSettings<AnimationRose> ValidateAndMap(
                    SpriteGrid sourceGrid, Tuple<MultiSettings<RoseTuple<ReadOnlyCollection<Vector2Int>>>, Vector2> selection
                )
                {
                    MultiSettings<AnimationRose> mapping = new MultiSettings<AnimationRose>();
                    foreach (KeyValuePair<State, RoseTuple<ReadOnlyCollection<Vector2Int>>> pair in selection.Item1)
                    {
                        ...
                        mapping[pair.Key] = ValidateAndMapAnimationRose(
                            ...
                            pair.Value.Right, selection.Item2
                        );
                    }
                ...
                private AnimationRose ValidateAndMapAnimationRose(
                    ...
                    ReadOnlyCollection<Vector2Int> right, Vector2 pivot
                )
                {
                    ...
                    Behaviours.SetObjectFieldValues(animationRose, new Dictionary<string, object>() {
                        { "up", MakeAnimation(from position in up select ValidateAndMapSprite(sourceGrid, position, pivot)) },
                        { "down", MakeAnimation(from position in down select ValidateAndMapSprite(sourceGrid, position, pivot)) },
                        { "left", MakeAnimation(from position in left select ValidateAndMapSprite(sourceGrid, position, pivot)) },
                        { "right", MakeAnimation(from position in right select ValidateAndMapSprite(sourceGrid, position, pivot)) },
                    });
                ...
                ~MultiRoseAnimatedPivotedSelection()
                {
                    if (result != null)
                    {
